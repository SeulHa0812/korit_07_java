...
//        int age = scanner.nextInt(); // 전에는 여기에 바로 작성해서 넣었는데,
        ...
        // 혹은 double 자료형을 넣을 수도 있음
        // 예외가 발생할 가능성이 있는 부분을 try 문 안에 넣어주는 것이 중요
        try {
        ...
        } catch (InputMismatchException e) { // Scanner 사용했는데 변수의 자료형과 일치하지 않을 때 작용됨 예) "스무살", 38.7
        ...
        /*
            ...
            콘솔 출력 순서를 고려할 필요가 있음
            예외가 발생하지 않는다면 예외 관련 객체가 생성되지도 않고,
            catch{} 문은 실행되지 않음.
            예외가 발생한다면
            예외 발생 시점 이후의 try{} 문은 실행되지 않고 곧장 catch{} 문이 실행됨
            그리고 예외 발생 유무와 관계 없이
            finally{} 문은 맨 마지막에 실행됨
         */
