...
//            System.out.println("ID 혹은 비밀번호를 확인하세요.");
            ...
            // argument로 하는 LoginFailedException 객체가 생성되었다고 할 수 있음
        }
    }
...
//        login(inputId, inputPassword); // 코드 작성 순서 잘 기억해야 함
        // throws 키워드를 달게 되니까 여기에 오류 발생
        try {
            login(inputId, inputPassword);
        } catch (LoginFailedException e) { // 자동완성으로 사용자 정의 Exception 클래스가 나왔는데
            // 그 근거는 login(0 메서드 다음에 throws LoginFailedException을 작성했기 때문
            System.out.println("로그인 실패 " + e.getMessage());
            // 이상을 실행했을 때 e.getMessage()의 결과값이 "ID 또는 PW가 틀렸습니다." 라는 것을 확인할 수 있음
        } finally {
...
/*
    ...
    try / catch / finally 와 throws / throw 가 추가 되었을 뿐
    method의 정의 / 생성자 정의 / 객체 생성 방식이라는 저희가 여태까지
    배워온 코드 작성 방법과 차이가 없다는 것을 확인할 수 있음
 */