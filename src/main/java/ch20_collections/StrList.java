...
public class StrList {
    public static void main(String[] args) {
        ...
        //List의 경우 인덱스 넘버 고정이 없지만, 배열의 경우 인덱스 넘버를 고정해줘야만 함.

        //2. 값을 대입하는 방법
        //배열
        strArray[0] = "김영";
        strArray[4] = "김사";
        //List -> 메서드를 통해 집어넣어야 함
        strList.add("java");
        ...
        /*
            이상의 이유는 List는 element가 명확하게 일치하는지를 확인함
            그래서 py가 python 이라는 String 데이터의 일부이기는 하지만 element 전체를 의미하지는
            않기 때문에 false 라는 결과값이 도출됨.
         */
        ...
        // 시험에 pop() 메서드가 나온적 있어서 비교
        // FIFO / .pop() -> 삭제 여부가 아니라 collection 에서 삭제하고 삭제된 element를 return함.
        System.out.println(strList);
        // List의 정렬
        Collections.sort(strList); // 배열 정렬할 때와 동일. 원본 List의 결과값을 바꿈
        System.out.println(strList);
